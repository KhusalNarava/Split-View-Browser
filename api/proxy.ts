export default async function handler(req, res) {
  ...
  try {
    ...
    // For HTML, inject base tag so relative assets load from the original domain
    ...
    const injectedScript = `
      ...
      <script>
        window.addEventListener('message', async (e) => {
          if (!e.data) return;
          if (e.data.type === 'FIND') {
            window.find(e.data.query, false, e.data.backwards, true, false, false, false);
          }
          if (e.data.type === 'SCREENSHOT') {
            try {
              if (window.html2canvas) {
                ...
                e.source.postMessage({ type: 'SCREENSHOT_RESULT', dataUrl: dataUrl }, '*');
              }
            } catch (err) {
              console.error('Screenshot inside iframe failed', err);
            }
          }
        });
      </script>
    ...
    if (text.includes('<head>')) {
      ...
      text = text.replace('<HEAD>', '<HEAD>' + injectedScript);
       ...
       text = '<head>' + injectedScript + '</head>' + text;
    }
